...
.mcmcamwg <- function(Model, Data, Iterations, Status, Thinning, Specs,
     ...
     LogFile)
     {
     # NOTE: LogFile is not passed.
     return(.Call("mcmcamwg", Model, Data, Iterations, Status, Thinning,
          Specs, Acceptance, Dev, DiagCovar, LIV, Mon, Mo0, ScaleF,
          thinned, tuning, PACKAGE="LaplacesDemonCpp"))
     }
...
.mcmcmwg <- function(Model, Data, Iterations, Status, Thinning, Specs,
     ...
     {
     # NOTE: LogFile is not passed.
     return(.Call("mcmcmwg", Model, Data, Iterations, Status, Thinning,
          Specs, Acceptance, Dev, DiagCovar, LIV, Mon, Mo0, ScaleF,
          thinned, tuning, PACKAGE="LaplacesDemonCpp"))
     }
...
.mcmcohss <- function(Model, Data, Iterations, Status, Thinning, Specs,
     ...
     if(A > 0)
          VarCov <- {VarCov*n + .Call("covar", post,
               PACKAGE="LaplacesDemonCpp")*nrow(post)} / {n+nrow(post)}
     ### Output
     out <- list(Acceptance=Iterations,
          Dev=Dev,
          DiagCovar=DiagCovar,
